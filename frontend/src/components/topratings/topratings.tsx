...
const TopRatings = () => {
    ...
    const [createdScore, setCreatedScore] = useState<Score | null>(null)
    ...
    const handleAccInput = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isNaN(chartIdInputValue) || Number(chartIdInputValue) < 0 || Number(chartIdInputValue) > 101) {
            ...
            setAccInputError("")
            if (forScoreChart) {
                setScoreRating(calculateRating(Number(forScoreChart.internalLevel), chartIdInputValue))
            }
        }
    }
